...
const ReviewData = ({ name, degree, rating, testimonial, imageUrl }) => {
  ...
  return (
    <div className="group before:max-mbl:w-[90vw] max-mbl:w-[90vw] mb-16 shadow-2xl shadow-[#00000060] border border-solid min-h-[430px] before:hover:scale-95 before:hover:h-96 before:hover:w-80 before:hover:rounded-b-2xl before:transition-all before:duration-500 before:content-[''] before:w-72 before:h-32 before:rounded-t-2xl before:bg-black before:py-5 before:absolute before:top-[-10px] after:top-[px] w-72 h-100 relative p-5 bg-white flex flex-col items-center justify-center gap-2 rounded-2xl">
      <img
        src={imageUrl}
        alt={name}
        className="h-[80px] object-cover mt-16 rounded-full border-5 border-slate-50 z-20 group-hover:scale-125 group-hover:-translate-x-24 group-hover:-translate-y-20 transition-all duration-500"
      />
      <div className="z-10 group-hover:-translate-y-10 transition-all duration-500 flex flex-col items-center justify-center">
        ...
        <p className="group-hover:text-sky-500 text-center font-normal px-3 pb-5 mt-2">{testimonial}</p>
      </div>
